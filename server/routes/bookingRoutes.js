...
const sendBookingEmail = async (bookingData) => {
  ...
  const ownerEmail = OWNER_EMAIL || 'mycarhubbbb@gmail.com';
...
const generateReceiptPDF = (bookingData, car = null) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.text(`For queries, contact support at ${process.env.OWNER_EMAIL || 'mycarhubbbb@gmail.com'} or call +91 9597693716`, 50, footerY + 30, { align: 'center', width: 495 });
...
const sendConfirmationEmail = async (bookingData) => {
  ...
  try {
    ...
    const toEmails = [bookingData.email, OWNER_EMAIL || 'mycarhubbbb@gmail.com'];
