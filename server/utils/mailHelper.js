...
export const sendMailHelper = async ({ to, replyTo, subject, html, attachments = [] }) => {
  ...
  const SENDGRID_FROM = process.env.SENDGRID_FROM?.trim() || 'mycarhubbbb@gmail.com';
  const EMAIL_USER = process.env.EMAIL_USER?.trim() || '';
  const EMAIL_PASS = process.env.EMAIL_PASS?.trim() || '';

  // Standardize recipient email address array and deduplicate to prevent duplicate email errors
